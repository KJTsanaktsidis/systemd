...
static int dhcp4_routing_policy_rule_handler(sd_netlink *rtnl, sd_netlink_message *m, Link *link) {
        ...
        r = sd_netlink_message_get_errno(m);
        if (r < 0 && r != -EEXIST) {
                log_link_message_warning_errno(link, m, r, "Could not set DHCPv4 routing policy rule: %m");
                link_enter_failed(link);
                return 1;
        }
...
static int dhcp4_add_source_routing_policy_rule(Link *link) {
        ...
        /* Only actually add the rule if we didn't already add the same rule from a
        previous lease. Otherwise, routing_policy_rule_configure will error with -EEXIST */
        if (
                link->dhcp4_source_routing_policy_rule &&
                routing_policy_rule_compare_func(link->dhcp4_source_routing_policy_rule, rule)
        ) {
                log_link_debug(link, "New and old source routing rules are the same, won't reconfigure");
                return 0;
        }
        /* Now, if we really are going to set up a new routing policy rule, free the old one.
...
static int dhcp4_remove_source_routing_policy_rule(Link *link, sd_dhcp_lease *lease, const struct in_addr *address, bool remove_all) {
        ...
        if (!remove_all && link->dhcp4_source_routing_policy_rule &&
        ...
        log_link_debug(
                link, "Removing source routing rule %s/%d -> * table %d pri %d family %d",
                from_addr_str,
                rule->from_prefixlen,
                rule->table,
                rule->priority,
